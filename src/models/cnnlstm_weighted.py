...
class CNN_LSTM_WEIGHTED(nn.Module):
    ...
    def train_model(self, trainloader, model, optimizer, num_epochs, checkpoint_epoch):
        ...
        """
        model.train(True)
        output_file = "weighted/losses_weighted2.txt"
        ...
        for epoch in range(checkpoint_epoch, num_epochs):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                if i % 10 == 0:    # print every 10 mini-batches
                    ...
                    running_loss = 0.0
            self.checkpoint(model, f"epoch_weighted/epoch-{epoch}.pth")
    ...
    def evaluate_model(self, dataloader, model, num_epochs):
        ...
        with torch.no_grad():
            ...
            with open("weighted/weighted_metrics2.txt", "w") as f:
        ...
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.savefig("weighted/weighted_loss_plot.png")
        plt.show()
...
model = CNN_LSTM_WEIGHTED(512*7*7 , 2, 1, regression = regression).to(device)
print(model)

 #Define the path to the saved model state dictionary
# path_to_saved_model = "epoch_weighted/epoch-27.pth"  # Replace with the actual path to your saved model
