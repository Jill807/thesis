...
class lstm_model(nn.Module):
    ...
    def train_model(self, model, n_epochs, train_X, train_y, val_X, val_y, test_X, test_y, optimizer, loss_fn, metrics):
        best_val_mse      = np.inf
        ...
        for epoch in range(n_epochs):
            ...
            with torch.no_grad():
                # Train
                y_pred     = model(train_X)
                train_mse  = loss_fn(y_pred, train_y)
                train_r2   = r2score(y_pred, train_y)
                # Validation
                y_pred     = model(val_X)
                val_mse    = loss_fn(y_pred, val_y)
                val_r2     = r2score(y_pred, val_y)

                # Test
                y_pred     = model(test_X)
                test_mse   = loss_fn(y_pred, test_y)
    ...
    def test_model(self, model, loss_fn, test_X, test_y):
        # Set to eval mode
        model.eval()
...
feature_columns = df.columns[:-9].values.tolist()
label_column    = "PM2.5"
lookback_window = 48
...
model = lstm_model(nr_input_features = nr_features, timestamps = TIMESTAMPS,
...
model.test_model(model, loss_fn, test_X, test_y)
